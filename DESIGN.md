...
## 10. Sang Tran Portfolio Guardrails

This project uses the PlayStation reference as inspiration, not as a clone. Keep the portfolio dark, cinematic, readable, and human.

### Non-Negotiable UI Rules
- Readability comes before mood. Body text on dark surfaces must be clearly legible at first glance.
- Do not place long text, descriptions, or paragraphs on top of project thumbnails.
- Thumbnails may contain only short badges or minimal metadata. Put title, role, year, and description below the image.
- Avoid decorative low-opacity background text unless it directly improves hierarchy. If it competes with real content, remove it.
- Every section needs one clear visual priority. Do not make heading, image, stats, and background all fight at once.
- Use fewer, larger cards over dense grids when showing portfolio work.
- Keep section spacing consistent: large desktop sections should breathe; do not allow sections to collide or overlap.
- Prefer simple dark surfaces, clean borders, and restrained shadows over glassmorphism-heavy panels.
- Mobile must be checked explicitly. A layout that only works on desktop is not done.

### QA Before Handoff
- Run `npm run qa:screenshots` after visual changes.
- Review the generated screenshots in `test-results/ui-screenshots/`.
- Capture desktop screenshots for Home, Portfolio, and About after visual changes.
