...
4. Treat every build warning as a failure. Fix the warning or clearly report the blocker instead of saying the build is clean.
5. If a browser shows a `.next/server/... Cannot find module` error after verification, stop the dev server, delete `.next`, then restart with `npm run dev`.
6. Do not leave generated cache/output files staged or modified. `.next`, `out`, coverage, test reports, logs, and temp files must stay ignored.
7. Final response must include what changed, the exact verification command, and whether lint/build passed with zero warnings.
