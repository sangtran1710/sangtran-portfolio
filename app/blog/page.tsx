          ...
          <div className="flex flex-col divide-y divide-zinc-800/60">
            {posts.map((post) => (
              <article key={post.slug} className="group py-8 first:pt-0">
                <Link href={`/blog/${post.slug}`} className="flex flex-col sm:flex-row gap-6 sm:gap-8 items-start">

                  {/* Thumbnail */}
                  {post.thumbnail && (
                    <div className="relative w-full sm:w-64 shrink-0 aspect-video rounded-lg overflow-hidden border border-zinc-800/50">
                      <Image
                        src={post.thumbnail}
                        alt={post.title}
                        fill
                      ...
                      />
                      <div className="absolute inset-0 bg-zinc-950/10 group-hover:bg-transparent transition-colors duration-300" />
                    </div>
                  )}
